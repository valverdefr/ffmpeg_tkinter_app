...
def show_format_options(file_type):
    if file_type == "video":
        format_options.set('avi')  # Set default format for video
        format_dropdown["menu"].delete(0, "end")
        for format_option in ['avi', 'mp4', 'mkv']:
            format_dropdown["menu"].add_command(label=format_option, command=tk._setit(format_options, format_option))
    elif file_type == "audio":
        format_options.set('mp3')  # Set default format for audio
        format_dropdown["menu"].delete(0, "end")
        for format_option in ['mp3', 'wav', 'ogg']:
            format_dropdown["menu"].add_command(label=format_option, command=tk._setit(format_options, format_option))
    elif file_type == "image":
        format_options.set('png')  # Set default format for image
        format_dropdown["menu"].delete(0, "end")
        for format_option in ['png', 'jpg', 'bmp']:
            format_dropdown["menu"].add_command(label=format_option, command=tk._setit(format_options, format_option))

# Conversion trigger handler
def on_convert():
    if selected_file_path and selected_file_type:
        selected_format = format_options.get()  # Get the selected format from dropdown
...
format_dropdown = tk.OptionMenu(root, format_options, "")  # Initialize with no options
