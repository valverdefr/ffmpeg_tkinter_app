...
def setup_ui(root, on_file_select, on_convert):
    ...
    # Set up the progress bar inside the status bar (segmented)
...
def update_status_bar(message, progress):
    ...
    segment_width = progress_bar_width // 7  # Divide into 7 segments
    filled_segments = int(progress / 100 * 7)  # Calculate how many segments should be filled

    for i in range(7):
        if i < filled_segments:
            progress_bar.create_rectangle(i * segment_width, 0, (i + 1) * segment_width, 20, fill="green")
        else:
            progress_bar.create_rectangle(i * segment_width, 0, (i + 1) * segment_width, 20, outline="grey")
