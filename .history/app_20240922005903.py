...
def process_video(overwrite=False):
    ...
    # Create a floating window for conversion progress
    progress_window = Toplevel(root)
    progress_window.title("Conversion in Progress")
    progress_label = tk.Label(progress_window, text="Converting to AVI...")
    progress_label.pack(pady=10)

    # Progress bar widget in the floating window
    ...
    cancel_button = tk.Button(progress_window, text="Cancel", command=lambda: cancel_conversion(progress_window))
    cancel_button.pack(pady=10)

    def cancel_conversion(window):
        # Close the window and cancel the conversion process
        window.destroy()
        messagebox.showinfo("Conversion Cancelled", "The conversion was cancelled.")
    
    def complete_conversion():
        # After conversion, update the UI and show success message
        cancel_button.destroy()  # Remove the Cancel button
        close_button = tk.Button(progress_window, text="Close", command=progress_window.destroy)
        close_button.pack(pady=10)
        progress_label.config(text="Conversion Complete! Output: .avi")
...
edit_menu.add_command(label="Convert", command=process_video, state="disabled")  # Disabled until a video is selected
convert_menu = edit_menu  # Save the reference for later
