...
def setup_ui(root, on_file_select, on_convert):
    ...
    custom_canvas.pack(fill=tk.X, pady=20)  # Allow the bar to stretch horizontally
    ...
    root.bind("<Configure>", on_resize)  # Bind the resize event to dynamically adjust the bar

    # Initially, update the UI with no file selected
    update_custom_progress_bar()  # Call this initially to draw the empty grooved bar
...
def update_custom_progress_bar(file_path=None):
    ...
    custom_canvas.update_idletasks()  # Force update to ensure drawing happens immediately
