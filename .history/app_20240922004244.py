...
def select_file():
    ...
    if file_path:
        ...
        update_custom_progress_bar()  # Redraw empty bar with grey border
...
def update_custom_progress_bar():
    ...
    # If no file is selected, display an empty bar with a light grey border
    if not selected_file_path:
        custom_canvas.create_rectangle(10, 10, window_width - 20, 80, outline="lightgrey", width=2)
    else:
...
# Initially draw the empty bar with a grey border
update_custom_progress_bar()
