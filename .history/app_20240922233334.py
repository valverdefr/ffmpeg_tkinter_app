...
def update_format_menu(file_type):
    ...
    if file_type == "video":
        for format_option in ['avi', 'mp4', 'mkv']:
            edit_menu.add_radiobutton(label=format_option, variable=selected_format, value=format_option)
        selected_format.set('avi')  # Default format for video
...
# Initialize the selected format variable after the root window is created
