...
# Functions for minimizing and maximizing the window
def minimize_window():
    root.wm_state('iconic')  # Minimizes the window

def maximize_window():
    root.wm_state('zoomed')  # Maximizes the window

def toggle_fullscreen():
    is_fullscreen = root.attributes("-fullscreen")
    root.attributes("-fullscreen", not is_fullscreen)  # Toggles full-screen mode
...
# Add buttons to control window state
btn_minimize = tk.Button(root, text="Minimize", command=minimize_window)
btn_minimize.pack(side=tk.LEFT, padx=10, pady=10)

btn_maximize = tk.Button(root, text="Maximize", command=maximize_window)
btn_maximize.pack(side=tk.LEFT, padx=10, pady=10)

btn_fullscreen = tk.Button(root, text="Toggle Fullscreen", command=toggle_fullscreen)
btn_fullscreen.pack(side=tk.LEFT, padx=10, pady=10)

# Add a button to select a video and process it
btn_process = tk.Button(root, text="Select and Process Video", command=process_video)
