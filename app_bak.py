...
def process_video(overwrite=False):
    ...
    time_label.config(text="Estimated time remaining: N/A")  # Show time estimate during conversion
    time_label.pack(side=tk.LEFT, padx=20)  # Make sure it's visible
    root.update_idletasks()

    def complete_conversion():
        # Ensure the final segment turns green
        draw_segmented_progress_bar(100)
        # After conversion, update the UI and show success message
        time_label.pack_forget()  # Hide the time estimate after completion
